        ...
        <nav className='bg-white w-1/4 rounded-3xl flex flex-col text-xl'>
            ...
            <ul>
                {routes
                    ...
                    .map(({path, name, icon: Icon}) => (
                        <li className='flex h-14 w-full justify-center border-b-2 hover:cursor-pointer' key={path}>
                            <NavLink
                                to={path}
                                className={({isActive}) =>
            ...
            {isAuthenticated && (
                <div className='mt-auto mb-4'>
                    <button className='flex h-14 items-center pl-4 w-full border-t-2' onClick={logout}>
                        <MdLogout/>
